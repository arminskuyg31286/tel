...
try {
    ...
    if (!$table_exists) {
        $result = $connect->query("CREATE TABLE setting (
            ...
            inline_keyboard ENUM('on','off') NOT NULL DEFAULT 'off'
        )
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_bin");
        if (!$result) {
        ...
        $active_phone_text = "0";
        $active_help = "0";
        $connect->query("INSERT INTO setting (Bot_Status, get_number, limit_usertest_all, time_usertest, val_usertest, help_Status, NotUser, namecustome, removedayc, status_verify, statuscategory, copy_cart, inline_keyboard) VALUES ('$active_bot_text', '$active_phone_text', '1', '1', '100', '$active_help', '0', '0', '1', '0', '1', '0', 'off')");
    } else {
        ...
        addFieldToTable($tableName, 'namecustome', 'user', "VARCHAR(200)");
        addFieldToTable($tableName, 'removedayc', '1', "VARCHAR(100)");
        addFieldToTable($tableName, 'inline_keyboard', 'off', "ENUM('on','off') NOT NULL");
...
try {
    ...
    if (!$table_exists) {
        ...
        if (mysqli_num_rows($Check_filde) != 1) {
           $result = $connect->query("ALTER TABLE invoice ADD Status VARCHAR(200)");
        }
        $Check_filde = $connect->query("SHOW COLUMNS FROM invoice LIKE 'sub_url'");
        if (mysqli_num_rows($Check_filde) != 1) {
            $result = $connect->query("ALTER TABLE invoice ADD sub_url VARCHAR(500)");
        }
    }
} catch (Exception $e) {
    file_put_contents("$randomString.txt", $e->getMessage());
...
try {
    ...
    if (!$table_exists) {
        ...
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_inline_key','{$textbotlang['Admin']['managepanel']['BoyService']}')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_support_qs','{$textbotlang['users']['support']['sendmessageuser']}')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_myid','{$textbotlang['users']['affiliates']['myid']}')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_affiliates','{$textbotlang['users']['affiliates']['btn']}')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_linkapp','{$textbotlang['users']['affiliates']['linkapp']}')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_BalanceBuy','{$textbotlang['users']['buy']['payandGet']}')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_discountcode','{$textbotlang['users']['buy']['discount']}')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_freetest','{$textbotlang['users']['Test-Service']}')");
        $connect->query("INSERT INTO textbot (id_text,text) VALUES ('text_info_test','{$textbotlang['users']['Text']['info_test']}')");
        ...
        $connect->query("INSERT IGNORE INTO textbot (id_text,text) VALUES ('text_inline_key','{$textbotlang['Admin']['managepanel']['BoyService']}')");
        $connect->query("INSERT IGNORE INTO textbot (id_text,text) VALUES ('text_support_qs','{$textbotlang['users']['support']['sendmessageuser']}')");
        $connect->query("INSERT IGNORE INTO textbot (id_text,text) VALUES ('text_myid','{$textbotlang['users']['affiliates']['myid']}')");
        $connect->query("INSERT IGNORE INTO textbot (id_text,text) VALUES ('text_affiliates','{$textbotlang['users']['affiliates']['btn']}')");
        $connect->query("INSERT IGNORE INTO textbot (id_text,text) VALUES ('text_linkapp','{$textbotlang['users']['affiliates']['linkapp']}')");
        $connect->query("INSERT IGNORE INTO textbot (id_text,text) VALUES ('text_BalanceBuy','{$textbotlang['users']['buy']['payandGet']}')");
        $connect->query("INSERT IGNORE INTO textbot (id_text,text) VALUES ('text_discountcode','{$textbotlang['users']['buy']['discount']}')");
        $connect->query("INSERT IGNORE INTO textbot (id_text,text) VALUES ('text_freetest','{$textbotlang['users']['Test-Service']}')");
        $connect->query("INSERT IGNORE INTO textbot (id_text,text) VALUES ('text_info_test','{$textbotlang['users']['Text']['info_test']}')");
...
try {
    ...
    if (!$table_exists) {
        ...
        $connect->query("INSERT INTO affiliates (description, status_commission, Discount, price_Discount, affiliatesstatus, affiliatespercentage) VALUES ('none','oncommission','onDiscountaffiliates','0','0','0')");
    }
} catch (Exception $e) {
    file_put_contents("$randomString.txt", $e->getMessage());
}
