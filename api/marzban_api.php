...
function token_panel($code_panel) {
    $code_panel = htmlspecialchars(trim($code_panel));
    ...
    if (!empty($panel['datelogin'])) {
        ...
        if (json_last_error() === JSON_ERROR_NONE && isset($date['time'])) {
            $elapsed = time() - strtotime($date['time']);
            if ($elapsed <= 3600 && !empty($date['access_token'])) {
                return $date;
            }
        }
    }
    $panel_url = rtrim($panel['url_panel'] ?? '', '/');
    if (!filter_var($panel_url, FILTER_VALIDATE_URL)) {
        return ['error' => 'Invalid panel URL'];
    }
    ...
    if (curl_errno($curl_token)) {
        $error_msg = htmlspecialchars(curl_error($curl_token));
    ...
    if (!empty($body['access_token'])) {
        ...
        update("marzban_panel", "datelogin", $data, 'name_panel', htmlspecialchars($panel['name_panel']));
    }
    return $body;
}

function token_panel_direct($url_panel, $username, $password) {
    $url_panel = filter_var(trim($url_panel), FILTER_SANITIZE_URL);
    $username  = htmlspecialchars(trim($username), ENT_QUOTES, 'UTF-8');
    $password  = htmlspecialchars(trim($password), ENT_QUOTES, 'UTF-8');
    if (!filter_var($url_panel, FILTER_VALIDATE_URL)) {
        return ['error' => 'Invalid panel URL'];
    }
    $url_get_token = rtrim($url_panel, '/') . '/api/admin/token';
    $data_token = [
        'username' => $username,
        'password' => $password
    ];
    ...
    $response = curl_exec($curl_token);
    if (curl_errno($curl_token)) {
        $error_message = htmlspecialchars(curl_error($curl_token), ENT_QUOTES, 'UTF-8');
        curl_close($curl_token);
        return ['error' => 'cURL error: ' . $error_message];
    }
    $http_code = curl_getinfo($curl_token, CURLINFO_HTTP_CODE);
    curl_close($curl_token);
    if ($http_code !== 200) {
...
function Get_System_Stats($location) {
    ...
    $tokenData = token_panel($panel['id']);
    $url = rtrim($panel['url_panel'], '/') . '/api/system';
    $headers = [
        'Accept: application/json',
...
function Restart_XrayCore($location) {
    $panel = select("marzban_panel", "*", "name_panel", $location, "select");
    $tokenData = token_panel($panel['id']);
    $url = rtrim($panel['url_panel'], '/') . '/api/core/restart';
    $headers = [
        'Accept: application/json',
        'Authorization: Bearer ' . $tokenData['access_token']
    ];
    ...
    curl_close($ch);
    return ['status' => 'ok', 'message' => "✅ هسته پنل <b>{$panel['name_panel']}</b> با موفقیت ری ‌استارت شد."];
}

function getuser($usernameac, $location) {
    $usernameac = htmlspecialchars(trim($usernameac), ENT_QUOTES, 'UTF-8');
    $location   = htmlspecialchars(trim($location), ENT_QUOTES, 'UTF-8');
    $panel = select("marzban_panel", "*", "name_panel", $location, "select");
    $Check_token = token_panel($panel['id']);
    $url =  $panel['url_panel'].'/api/user/' . $usernameac;
    $header_value = 'Bearer ';
    $ch = curl_init();
...
function adduser($username, $expire, $data_limit, $location, $is_test = false) {
    ...
    $data = [
        "proxies" => json_decode($panel['proxies']),
        "data_limit" => $data_limit,
        "username" => $username
    ];
    if (is_marzban_version_above_084($location)) {
        $group_name = $is_test ? "group_test" : "group1";
        $groups = get_groups($location);
        $group_id = null;
        if (is_array($groups) && isset($groups['groups'])) {
            foreach ($groups['groups'] as $group) {
                if (isset($group['name'], $group['id']) && $group['name'] === $group_name) {
                    $group_id = $group['id'];
                    break;
                }
            }
    ...
    if (!empty($panel['inbounds']) && $panel['inbounds'] !== "null") {
        $data['inbounds'] = json_decode($panel['inbounds'], true);
    }
    if ($expire == "0") {
        $data['expire'] = 0;
    } else {
        if ($panel['onholdstatus'] === "ononhold") {
            $data["expire"] = 0;
            $data["status"] = "on_hold";
...
function ResetUserDataUsage($usernameac, $location) {
    $panel = select("marzban_panel", "*", "name_panel", $location, "select");
    $tokenData = token_panel($panel['id']);
    $url = rtrim($panel['url_panel'], '/') . '/api/user/' . $usernameac . '/reset';
    $headers = [
        'Accept: application/json',
        'Authorization: Bearer ' . $tokenData['access_token']
    ];
...
function ensure_default_groups($location) {
    ...
    try {
        ...
        $required_groups = array('group1', 'group_test');
...
// دریافت وضعیت HWID کاربر - نسخه اصلاح شده و مستقل
function getUserHWIDStatus($location, $username) {
    $panel = select("marzban_panel", "*", "name_panel", $location, "select");
    if (!$panel || !is_array($panel)) {
        return ['used' => 0, 'limit' => 0, 'error' => 'Panel not found'];
    }

    $tokenData = token_panel($panel['id']);
    if (isset($tokenData['error'])) {
        return ['used' => 0, 'limit' => 0, 'error' => $tokenData['error']];
    }

    $baseUrl = rtrim($panel['url_panel'], '/');

    // دریافت اطلاعات کاربر برای گرفتن user_id و hwid_limit
    $userInfoUrl = "$baseUrl/api/user/" . urlencode($username);
    ...
    // گرفتن لیست HWIDها
    $hwidUrl = "$baseUrl/api/user/$userId/hwids";
    
    $ch = curl_init($hwidUrl);
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_HTTPHEADER => [
            'Authorization: Bearer ' . $tokenData['access_token'],
    ...
    $hwidResponse = curl_exec($ch);
    curl_close($ch);

    $hwidData = json_decode($hwidResponse, true);
    
    $used = 0;
    if (isset($hwidData['hwids']) && is_array($hwidData['hwids'])) {
        $used = count($hwidData['hwids']);
    } elseif (isset($hwidData['count'])) {
        $used = (int)$hwidData['count'];
    }

    // حد مجاز HWID
    $limit = isset($userData['hwid_limit']) && $userData['hwid_limit'] > 0 
        ? (int)$userData['hwid_limit'] 
        : 2; // پیش‌فرض
